function [OutletAir,OutletWater,TotHeatTransferRate,SenWaterCoolingCoilRate]=...
    SimpleDesignCoolingCoilLMTD(...
    InletAir,InletWater,TempSetPoint,SurfaceArea,Schedule,DesInformation,Parameter,DesInletAir,DesInletWater)
...
MaxWaterMassFlowrate=1.4*1e3*DesWaterFlowrate;
...
while i<=Row
    ...
    if((Schedule(i)~=0.0)&&(AirMassFlowRate(i)>0)&& (SurfaceArea>0))
        ...
        LMTD(i,:)=TotHeatTransferRate(i)./SurfaceArea./TotalU(i);
        ...
        % Calculate the outlet water temperature by fitting LMTD function.
        t1=OutletAirTemp(i)-InletWaterTemp;
        
        if isnan(LMTD(i))|isinf(LMTD(i))
            LMTD(i)=0.5;
        end
        
        c1=[0.001891,-0.04092,0.5711,0.4314-LMTD(i)./max(0,t1)]; % the fitting coefficients in polynomial equations
        ...
        if anyreal(r)
            ...
            OutletWaterTemp(i)=InletAirTemp(i)-t1*x;
            
            % Then the massflow rate on water side.
            WaterMassFlowRate(i,:)=TotHeatTransferRate(i)./WaterCp(i)./...
                (OutletWaterTemp(i)-InletWaterTemp(i)); % Kg/s
            
            % ************check if water mass flowrate is reasonable*******
            
            if ((WaterMassFlowRate(i,:)>=MinWaterMassFlowrate)&(WaterMassFlowRate(i,:)<=MaxWaterMassFlowrate))
                ...
                WaterMassFlowRate(i,:)=min(max(WaterMassFlowRate(i,:),...
                    MinWaterMassFlowrate),MaxWaterMassFlowrate);
                
                % The new heat transfer question has become the one with given inlet conditions;
                InletWater.flowrate=WaterMassFlowRate(i,:)./1e3;
                UA.external=ExternalU(i)*SurfaceArea;
                UA.internal=InternalU(i)*SurfaceArea;
                UA.total=TotalU(i)*SurfaceArea;
                ...
                [OutletAirR,OutletWaterR,TotHeatTransferRate(i),SenWaterCoolingCoilRateR]=...
                    CoolingCoilLMTD(InletAir,InletWater,UA,...
                    Schedule(i),HeatExchType,AnalysisMode,DesInletAir,DesInletWater,Parameter);
                
                OutletAirTemp(i)=OutletAirR.temp;
                OutletAirRH(i)=OutletAirR.RH;
                OutletAirHumidityRatio(i)=OutletAirR.W;
                OutletAirEnthalpy(i)=OutletAirR.H;
                
                OutletWaterTemp(i)=OutletWaterR.temp;
                

            SenWaterCoolingCoilRate(i)=SenWaterCoolingCoilRateR;
            
            AirPressureDrop(i)=OutletAirR.pressure_loss;

            WaterPressureDrop(i)=OutletWaterR.pressure_loss;
            end
