function [OutletAir,OutletWater,TotHeatTransferRate]=SimpleDesignHeatingCoil(...
    InletAir,InletWater,TempSetPoint,WaterTempDifference,Schedule,Parameter)
% Simplified Heating coil for design process.
%   %
...
while i<=Row
    ...
    if((Schedule(i)~=0.0)&&(AirMassFlowRate(i)>0))
            ...
            TotHeatTransferRate(i,:)=AirMassFlowRate(i).*AirCp(i)...
                .*(InletAirTemp(i)-OutletAirTemp(i));
           
        
        % Calculate the total U value. Ignore the change of water side U value
        % duing operating phase.
        %TotalU(i,:)=-A*AirVelocity(i).^m.*WaterVelocity(i).^n;
        ...
        %fun=@LMTDEquation;
        %OutletWaterTemp(i,:)=fsolve(fun,InletWaterTemp(i)-5);
        OutletWaterTemp(i,:)=InletWaterTemp(i)-WaterTempDifference(i);
