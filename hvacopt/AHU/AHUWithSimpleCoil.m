function [OutletAirAHU,OutletWaterCC,OutletWaterHC,PowerAHU,TotHeatTransferRate,EffectivenessFan,EconomicCost]=...
    AHUWithSimpleCoil(InletAir,InletWaterCC,InletWaterHC,AirSetPointTemp,WaterTempDifference,Schedule,...
...
if nargin<8
    ...
    ParameterAHU(3).DesCoolingCoil.ParameterCC.AirResis=18.5;
    ParameterAHU(3).DesCoolingCoil.ParameterCC.WaterResis=18.5;
...
WaterTempDifferenceCC=WaterTempDifference.CC;
WaterTempDifferenceHC=WaterTempDifference.HC;
    ...
    if (Schedule(k)~=0)&&(AirFlowRate(k)>0)
        if (InletAirTemp(k)-AirSetPointTemp(k)>=0.5)                        % START cooling coil;
            ...
            [OutletAirCC,OutletWaterCC,TotWaterCoolingCoilRateCC,SenWaterCoolingCoilRateCC]...
                =SimpleDesignCoolingCoil(InletAirCC,InletWaterCC,OutletAirTempSetPoint,WaterTempDifferenceCC,...
                Schedule(k),ParameterCC);
        ...
        elseif (InletAirTemp(k)-AirSetPointTemp(k)<=-0.5)% Start heating coil
            ...
            [OutletAirHC,OutletWaterHC,TotHeatingCoilRate]...
                ...
                WaterTempDifferenceHC,Schedule(k),ParameterHC);
